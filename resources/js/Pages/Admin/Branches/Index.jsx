...
import { PageHeader, DataTable, Btn, Card, StatusBadge, FlashMessages, T } from '@/Components/TurnosUI';
...
export default function BranchesIndex({ branches }) {
    ...
    const columns = [
        ...
        { key: 'code', label: 'Código', render: r => <span style={{ background: T.accentSoft, color: T.accent, padding: '2px 8px', borderRadius: 4, fontSize: 11, fontWeight: 600 }}>{r.code}</span> },
        { key: 'city', label: 'Ciudad' },
        { key: 'phone', label: 'Teléfono' },
        { key: 'is_active', label: 'Estado', render: r => (
            <span style={{ color: r.is_active ? T.success : T.danger, fontWeight: 600, fontSize: 11 }}>
                {r.is_active ? '● Activa' : '○ Inactiva'}
            </span>
        )},
        { key: 'is_open', label: 'Abierta', render: r => (
            <span style={{ color: r.is_open ? T.success : T.textMuted, fontSize: 11 }}>
                {r.is_open ? '🟢 Abierta' : '⚫ Cerrada'}
            </span>
        )},
        { key: 'today_tickets', label: 'Hoy', align: 'center', render: r => (
            <span style={{ fontWeight: 700, fontVariantNumeric: 'tabular-nums' }}>{r.today_tickets} <span style={{ color: T.textMuted, fontWeight: 400 }}>/ {r.max_daily_tickets}</span></span>
        )},
