...
export default function QueuesIndex({ queues, branches, currentBranchId }) {
    ...
    const columns = [
        { key: 'prefix', label: 'Prefijo', render: r => <span style={{ fontFamily: 'monospace', fontSize: 14, fontWeight: 700, color: theme.accent }}>{r.prefix}</span> },
        ...
        { key: 'waiting_count', label: 'En espera', align: 'center', render: r => <span style={{ fontWeight: 700, color: r.waiting_count > 0 ? theme.warning : theme.textMuted }}>{r.waiting_count}</span> },
        ...
        { key: 'is_active', label: 'Estado', render: r => <span style={{ color: r.is_active ? theme.success : theme.danger, fontSize: 11, fontWeight: 600 }}>{r.is_active ? '● Activa' : '○ Inactiva'}</span> },
