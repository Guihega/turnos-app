...
import { PageHeader, DataTable, Btn, Select, FlashMessages, theme } from '@/Components/TurnosUI';

export default function CountersIndex({ counters, branches, currentBranchId }) {
    const { flash } = usePage().props;
    const statusColors = { open: theme.success, serving: theme.accent, paused: theme.warning, closed: theme.textMuted };
    const statusLabels = { open: 'Disponible', serving: 'Atendiendo', paused: 'Pausada', closed: 'Cerrada' };

    const columns = [
        { key: 'number', label: '#', render: r => <span style={{ fontSize: 18, fontWeight: 800, fontVariantNumeric: 'tabular-nums', color: theme.accent }}>{r.number}</span> },
        ...
        { key: 'operator_name', label: 'Operador', render: r => r.operator_name || <span style={{ color: theme.textMuted }}>Sin asignar</span> },
        { key: 'current_ticket', label: 'Turno actual', render: r => r.current_ticket ? <span style={{ fontWeight: 600, fontVariantNumeric: 'tabular-nums' }}>{r.current_ticket}</span> : <span style={{ color: theme.textMuted }}>—</span> },
