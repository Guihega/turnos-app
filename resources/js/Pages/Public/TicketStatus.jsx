...
export default function TicketStatus({ branch, ticket }) {
    ...
    const vibrate = () => { try { navigator.vibrate?.([200, 100, 200, 100, 400]); } catch {} };
        ...
        'TicketCalled': () => {
            ...
            router.reload({ only: ['ticket'], onSuccess: (page) => {
                ...
                if (t?.status !== prev) {
                    ...
                    if (t.status === 'called') { setJustCalled(true); vibrate(); }
                }
            }});
        },
        'TicketCompleted': () => {
    ...
    useEffect(() => {
        ...
        const check = () => { const state = window.Echo?.connector?.pusher?.connection?.state; setWsConnected(state === 'connected'); };
        check();
    ...
    const logoRadius = branding.logo_shape === 'circle' ? '50%' : branding.logo_shape === 'rounded' ? 10 : 4;

    return (<>
        <Head title={`Turno ${ticket.display_number}`} />
        <div style={{
            fontFamily: T.font, background: T.bg, color: T.text, minHeight: '100dvh',
            display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
            padding: '24px 16px', ...cssVars,
        }}>
            <div style={{ maxWidth: 400, width: '100%', textAlign: 'center' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, marginBottom: 24 }}>
                    {logoUrl ? (
                        <img src={logoUrl} alt={tenantName} style={{ width: 26, height: 26, borderRadius: logoRadius, objectFit: 'cover' }} />
                    ) : (
                        <div style={{ width: 26, height: 26, borderRadius: 7, background: `linear-gradient(135deg, ${primaryColor}, ${branding.secondary_color || '#8B5CF6'})`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 900, fontSize: 10, color: '#fff' }}>{(tenantName || 'O')[0]}</div>
                    )}
                    <span style={{ fontSize: 13, fontWeight: 600, color: T.textMuted }}>{branch.name}</span>
                    <div style={{
                        width: 6, height: 6, borderRadius: '50%', marginLeft: 2,
                        background: wsConnected ? '#10B981' : T.textMuted,
                        animation: wsConnected ? 'statusBlink 2s infinite' : 'none',
                    }} title={wsConnected ? 'Tiempo real' : 'Actualización periódica'} />
                </div>

                {/* ══ CALLED STATE — Full prominence ══ */}
                {justCalled && ticket.status === 'called' && (
                    <div style={{
                        background: `linear-gradient(135deg, ${primaryColor}, color-mix(in srgb, ${primaryColor} 70%, #6366F1))`,
                        borderRadius: 24, padding: '36px 24px', marginBottom: 18,
                        animation: 'calledPulse 2s ease-in-out infinite',
                        boxShadow: `0 8px 40px color-mix(in srgb, ${primaryColor} 40%, transparent)`,
                    }}>
                        <div style={{ fontSize: 44, marginBottom: 10 }}>📢</div>
                        <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.7)', textTransform: 'uppercase', letterSpacing: '0.2em', fontWeight: 700, marginBottom: 8 }}>¡Es tu turno!</div>
                        <div style={{ fontSize: 52, fontWeight: 900, fontFamily: T.mono, color: '#fff', letterSpacing: '-0.04em', lineHeight: 1, marginBottom: 12 }}>
                            {ticket.display_number}
                        </div>
                        <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.7)', marginBottom: 18 }}>Dirígete a la ventanilla</div>
                        <div style={{
                            display: 'inline-block', background: 'rgba(255,255,255,0.15)', borderRadius: 18, padding: '14px 36px',
                            backdropFilter: 'blur(10px)',
                        }}>
                            <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.6)', textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 4, fontFamily: T.mono }}>Ventanilla</div>
                            <div style={{ fontSize: 48, fontWeight: 900, color: '#fff', fontFamily: T.mono, lineHeight: 1 }}>{ticket.counter_number || '—'}</div>
                        </div>
                    </div>
                )}
                ...
                {!(justCalled && ticket.status === 'called') && (
                    <div style={{
                        background: T.card, borderRadius: 22, padding: '32px 24px 28px', border: `1px solid ${T.border}`,
                        ...
                        <div style={{ fontSize: 38, marginBottom: 10, transition: 'transform 0.4s', transform: pulse ? 'scale(1.15)' : 'scale(1)' }}>{cfg.icon}</div>

                        <div style={{ fontSize: 9, color: T.textMuted, textTransform: 'uppercase', letterSpacing: '0.15em', marginBottom: 8, fontFamily: T.mono }}>Tu turno</div>

                        <div style={{
                            fontSize: 44, fontWeight: 900, fontFamily: T.mono, letterSpacing: '-0.04em',
                            color: cfg.color, lineHeight: 1, marginBottom: 14,
                            animation: pulse ? 'statusScaleIn 0.5s ease' : 'none',
                        }}>
                            {ticket.display_number}
                        </div>
                        ...
                        <div style={{
                            ...
                            fontSize: 12, fontWeight: 700, marginBottom: 16, letterSpacing: '0.02em',
                        }}>
                            {cfg.label}
                        </div>

                        <p style={{ fontSize: 13, color: T.textMuted, lineHeight: 1.6, marginBottom: 18, maxWidth: 280, margin: '0 auto 18px' }}>{cfg.msg}</p>

                        {/* Counter (for in_progress) */}
                        {ticket.status === 'in_progress' && ticket.counter_number && (
                            <div style={{ background: T.bg, borderRadius: 14, padding: '16px 20px', display: 'inline-block' }}>
                                <div style={{ fontSize: 9, color: T.textMuted, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 4, fontFamily: T.mono }}>Ventanilla</div>
                                <div style={{ fontSize: 44, fontWeight: 900, color: T.blue, fontFamily: T.mono, lineHeight: 1 }}>{ticket.counter_number}</div>
                            </div>
                        )}
                        ...
                        {ticket.status === 'waiting' && (
                            <div style={{ display: 'flex', gap: 10, justifyContent: 'center', marginTop: 4 }}>
                                {ticket.position != null && (
                                    <div style={{ background: T.bg, borderRadius: 12, padding: '12px 22px' }}>
                                        <div style={{ fontSize: 26, fontWeight: 900, color: T.amber, fontFamily: T.mono }}>{ticket.position}</div>
                                        <div style={{ fontSize: 8, color: T.textMuted, textTransform: 'uppercase', letterSpacing: '0.08em', fontFamily: T.mono }}>en la fila</div>
                                    </div>
                                )}
                                {ticket.estimated_wait_minutes != null && (
                                    <div style={{ background: T.bg, borderRadius: 12, padding: '12px 22px' }}>
                                        <div style={{ fontSize: 26, fontWeight: 900, fontFamily: T.mono }}>~{ticket.estimated_wait_minutes}</div>
                                        <div style={{ fontSize: 8, color: T.textMuted, textTransform: 'uppercase', letterSpacing: '0.08em', fontFamily: T.mono }}>min</div>
                                    </div>
                                )}
                            </div>
                        )}
                        ...
                        {ticket.status === 'transferred' && ticket.new_ticket && (
                            <div style={{ background: T.bg, borderRadius: 14, padding: 18, border: `1px solid color-mix(in srgb, var(--t-cyan, #06B6D4) 15%, transparent)`, marginTop: 12 }}>
                                <div style={{ fontSize: 9, color: T.textMuted, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 6, fontFamily: T.mono }}>Nuevo turno</div>
                                <div style={{ fontSize: 30, fontWeight: 900, color: 'var(--t-cyan, #06B6D4)', fontFamily: T.mono, marginBottom: 4 }}>{ticket.new_ticket.display_number}</div>
                ...
                {/* ── Ticket details ── */}
                <div style={{ background: T.card, borderRadius: 14, padding: '14px 18px', border: `1px solid ${T.border}`, textAlign: 'left' }}>
                    {[
                        ...
                        <div key={i} style={{
                            ...
                            borderBottom: i < arr.length - 1 ? `1px solid color-mix(in srgb, ${T.border} 50%, transparent)` : 'none',
                        }}>
                            <span style={{ fontSize: 11, color: T.textMuted }}>{item.l}</span>
                            <span style={{ fontSize: 11, fontWeight: 600, fontFamily: T.mono }}>{item.v}</span>
                        </div>
                    ))}
                </div>
                ...
                {!done && (
                    <div style={{ marginTop: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                        <div style={{ width: 5, height: 5, borderRadius: '50%', background: wsConnected ? '#10B981' : T.amber, animation: 'statusBlink 2s infinite' }} />
            ...
            <style>{`
                @keyframes statusScaleIn { from { transform: scale(.94); opacity: 0; } to { transform: scale(1); opacity: 1; } }
