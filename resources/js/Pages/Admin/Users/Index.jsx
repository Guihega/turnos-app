    ...
    const columns = [
        { key: 'name', label: 'Nombre', render: r => (
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{ width: 32, height: 32, borderRadius: '50%', background: roleColors[r.role] || theme.accent, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 700, fontSize: 13, flexShrink: 0 }}>{r.name.charAt(0)}</div>
                <div><div style={{ fontWeight: 600 }}>{r.name}</div><div style={{ fontSize: 11, color: theme.textMuted }}>{r.email}</div></div>
            </div>
        )},
        { key: 'role_label', label: 'Rol', render: r => (
            <span style={{ background: (roleColors[r.role] || theme.accent) + '18', color: roleColors[r.role] || theme.accent, padding: '3px 8px', borderRadius: 4, fontSize: 11, fontWeight: 600 }}>{r.role_label}</span>
        )},
        { key: 'branches', label: 'Sucursales', render: r => (
            <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                {r.branches.map(b => <span key={b.id} style={{ background: theme.accentSoft, color: theme.accent, padding: '2px 6px', borderRadius: 3, fontSize: 10 }}>{b.name}</span>)}
                {r.branches.length === 0 && <span style={{ color: theme.textMuted, fontSize: 11 }}>—</span>}
            </div>
        )},
        ...
        { key: 'last_login_at', label: 'Último acceso', render: r => <span style={{ fontSize: 11, color: theme.textMuted }}>{r.last_login_at || 'Nunca'}</span> },
